...
export class ContextBuilder {
  ...
  buildReport(report: VerificationReport): string {
    ...
    const verdictLabel =
      verdict === "TRUST" ? "PASS" : verdict === "REVIEW" ? "REVIEW" : "REJECT";
    ...
    if (diffResults.length > 0) {
      ...
      for (const diff of limitedDiffs) {
        parts.push(
          `- ${diff.filePath}: ${diff.summary || "N/A"} (risk: ${diff.riskScore})`,
        );
      }
      if (diffResults.length > limitedDiffs.length) {
        parts.push(
          `  ... and ${diffResults.length - limitedDiffs.length} more files`,
        );
      }
      parts.push("");
    }
    ...
    if (report.hallucinations.length > 0) {
      ...
      const critical = report.hallucinations.filter(
      ...
      const prioritized = [
        ...
        ...high.slice(
          0,
          Math.max(0, this.budget.maxHallucinationReports - critical.length),
        ),
        ...other.slice(
          0,
          Math.max(
            0,
            this.budget.maxHallucinationReports - critical.length - high.length,
          ),
        ),
      ];
      ...
      for (const issue of prioritized) {
        ...
        const location =
          issue.location || issue.line ? ` (line ${issue.line})` : "";
    ...
    // 置信度维度
    if (
      this.budget.includeConfidenceDimensions &&
      report.confidence.dimensions
    ) {
    ...
    if (report.confidence.riskFactors.length > 0) {
      ...
      for (const factor of report.confidence.riskFactors.slice(
        0,
        this.budget.maxRiskFactors,
      )) {
        parts.push(`- ${factor}`);
      }
      parts.push("");
    ...
    if (result.length > this.budget.maxOutputChars) {
      result =
        result.substring(0, this.budget.maxOutputChars - 20) +
        "\n\n_[truncated]_";
    }
  ...
  buildHallucinationReport(
    ...
    const limited = hallucinations.slice(
      0,
      this.budget.maxHallucinationReports,
    );
