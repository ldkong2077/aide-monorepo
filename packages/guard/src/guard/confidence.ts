...
export class ConfidenceScorer {
  ...
  computeScore(
    ...
    // 判定结果
    const verdict = this.determineVerdict(overall);

    // 生成风险因素
    const riskFactors = this.generateRiskFactors(diffResult, hallucinations);
   ...
   * 基于幻觉检测的数量和严重程度
   */
  private computeHallucinationFreeScore(hallucinations: HallucinationReport[]): number {
  ...
  private computeTestPassRateScore(testResult?: TestResult): number {
    if (!testResult || testResult.total === 0) return 50;
