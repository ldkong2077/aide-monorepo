...
describe("TenantCostTracker", () => {
  ...
  describe("config validation", () => {
    it("rejects non-positive budgetDaily", () => {
      expect(() => new TenantCostTracker({ budgetDaily: 0 })).toThrow(
        /budgetDaily/,
      );
      expect(() => new TenantCostTracker({ budgetDaily: -1 })).toThrow(
        /budgetDaily/,
      );
    });

    it("rejects alertThreshold out of (0, 1]", () => {
      expect(() => new TenantCostTracker({ alertThreshold: 0 })).toThrow(
        /alertThreshold/,
      );
      expect(() => new TenantCostTracker({ alertThreshold: 1.5 })).toThrow(
        /alertThreshold/,
      );
