...
function mockRedis() {
  ...
  return {
    ...
    async scan(
      ...
      const p = pattern.replace("*", "");
      const keys = [...store.keys()].filter((k) => k.startsWith(p));
...
describe("RedisTokenBucketRateLimiter", () => {
  ...
  beforeEach(() => {
    limiter = new RedisTokenBucketRateLimiter(mockRedis() as any, {
      limit: 5,
      windowMs: 10000,
    });
  });
  ...
  it("refills over time", async () => {
    await limiter.check("c", 1000);
    await limiter.check("c", 1000);
    expect((await limiter.check("c", 6000)).allowed).toBe(true);
  });

  it("isolates keys", async () => {
    await limiter.check("x", 1000);
    await limiter.check("x", 1000);
    expect((await limiter.check("y", 1000)).remaining).toBe(4);
  });

  it("getConfig works", () => {
    const c = limiter.getConfig();
    expect(c.limit).toBe(5);
    expect(c.windowMs).toBe(10000);
  });
  it("size works", async () => {
    await limiter.check("s1", 1000);
    await limiter.check("s2", 1000);
    expect(await limiter.size()).toBe(2);
  });
  it("reset works", async () => {
    await limiter.check("r", 1000);
    await limiter.reset("r");
    expect(await limiter.size()).toBe(0);
  });
  it("resetAll works", async () => {
    await limiter.check("a", 1000);
    await limiter.check("b", 1000);
    await limiter.resetAll();
    expect(await limiter.size()).toBe(0);
  });
  it("throws on invalid config", () => {
    expect(
      () => new RedisTokenBucketRateLimiter(mockRedis() as any, { limit: 0 }),
    ).toThrow("> 0");
    expect(
      () =>
        new RedisTokenBucketRateLimiter(mockRedis() as any, { windowMs: 0 }),
    ).toThrow("> 0");
  });
});
