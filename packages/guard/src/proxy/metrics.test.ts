...
describe('createMetrics', () => {
  ...
  it('aide_http_request_duration_seconds observes a value into the right bucket', async () => {
    const m = createMetrics();
    m.httpDuration.observe({ method: 'POST', route: '/v1/chat/completions' }, 0.12);
    const text = await m.toText();
  ...
  it('aide_upstream_requests_total is labelled by provider, model, and outcome', async () => {
    const m = createMetrics();
    m.upstreamRequests.inc({ provider: 'openai', model: 'gpt-4o-mini', outcome: 'success' });
    m.upstreamRequests.inc({ provider: 'openai', model: 'gpt-4o-mini', outcome: 'timeout' });
    m.upstreamRequests.inc({ provider: 'openai', model: 'gpt-4o-mini', outcome: 'error' });

    const text = await m.toText();
    expect(text).toMatch(/aide_upstream_requests_total\{provider="openai",model="gpt-4o-mini",outcome="success"\}\s+1/);
    expect(text).toMatch(/aide_upstream_requests_total\{provider="openai",model="gpt-4o-mini",outcome="timeout"\}\s+1/);
    expect(text).toMatch(/aide_upstream_requests_total\{provider="openai",model="gpt-4o-mini",outcome="error"\}\s+1/);
  });
