  ...
  return {
    async eval(script: string, _numKeys: number, ...args: string[]) {
      if (script.includes("INCRBYFLOAT")) {
        ...
        const spendKey = args[0],
          openKey = args[1];
        const inc = Number(args[2]),
          threshold = Number(args[3]),
          _ttl = Number(args[4]);
...
describe("RedisTenantCostTracker", () => {
  ...
  beforeEach(() => {
    tracker = new RedisTenantCostTracker(mockRedis() as any, {
      budgetDaily: 10,
      alertThreshold: 0.8,
    });
  });
  ...
  it("getConfig works", () => {
    ...
    expect(c.budgetDaily).toBe(10);
    expect(c.alertThreshold).toBe(0.8);
  });
  it("size works", async () => {
    await tracker.record("s1", 1, 1000);
    await tracker.record("s2", 1, 1000);
    expect(await tracker.size()).toBe(2);
  });
  it("reset works", async () => {
    await tracker.record("r", 5, 1000);
    await tracker.reset("r");
    expect(await tracker.isCircuitOpen("r")).toBe(false);
  });
  it("resetAll works", async () => {
    await tracker.record("a", 1, 1000);
    await tracker.record("b", 1, 1000);
    await tracker.resetAll();
    expect(await tracker.size()).toBe(0);
  });

  it("snapshotAll returns all", async () => {
    await tracker.record("s1", 2, 1000);
    await tracker.record("s2", 8, 1000);
  ...
  it("throws on invalid config", () => {
    expect(
      () => new RedisTenantCostTracker(mockRedis() as any, { budgetDaily: 0 }),
    ).toThrow("> 0");
    expect(
      () =>
        new RedisTenantCostTracker(mockRedis() as any, { alertThreshold: 0 }),
    ).toThrow("must be in");
  });
});
