...
describe('parseRawSSEStream', () => {
  it('parses a single OpenAI-style event', async () => {
    const events = collect(
      parseRawSSEStream(chunksToBytes(['data: {"id":"abc","object":"chat.completion.chunk"}\n\n'])),
    );
    expect(await events).toEqual([{ data: '{"id":"abc","object":"chat.completion.chunk"}' }]);
  });

  it('parses an Anthropic-style event with event: prefix', async () => {
    const events = collect(
      parseRawSSEStream(
        chunksToBytes(['event: message_start\ndata: {"type":"message_start"}\n\n']),
      ),
    );
    expect(await events).toEqual([{ event: 'message_start', data: '{"type":"message_start"}' }]);
  });
  ...
  it('concatenates multi-line data: fields with newline separator', async () => {
    const events = collect(
      parseRawSSEStream(chunksToBytes(['data: line1\ndata: line2\ndata: line3\n\n'])),
    );
...
describe('extractTokenUsageFromSSE (OpenAI)', () => {
  it('returns null when no events contain usage', () => {
    const events: SSEEvent[] = [{ data: '{"id":"abc"}' }, { data: '{"choices":[]}' }];
