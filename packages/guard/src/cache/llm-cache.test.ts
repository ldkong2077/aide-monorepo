...
describe('computeRequestHash', () => {
  ...
  it('is order-independent across object keys (canonicalisation)', () => {
    ...
    const a = { tools: [{ type: 'function', function: { name: 'f', parameters: { b: 1, a: 2 } } }] };
    const b = { tools: [{ type: 'function', function: { name: 'f', parameters: { a: 2, b: 1 } } }] };
    expect(computeRequestHash('m', messages, a)).toBe(computeRequestHash('m', messages, b));
  });
...
describe('LLMCache', () => {
  ...
  describe('TTL expiry', () => {
    it('expires entries past their TTL', async () => {
      const shortTtlCache = new LLMCache({ dbPath: makeTmpDbPath('-ttl'), defaultTtlMs: 5 });
      try {
    ...
    it('respects a 0 TTL (no expiry)', async () => {
      const noTtlCache = new LLMCache({ dbPath: makeTmpDbPath('-no-ttl'), defaultTtlMs: 0 });
      try {
        const key = computeRequestHash('m', [{ role: 'user', content: 'q' }]);
  ...
  describe('size cap & LRU eviction', () => {
    it('evicts the least-recently-accessed entries past the cap', () => {
      const smallCache = new LLMCache({ dbPath: makeTmpDbPath('-lru'), maxEntries: 3 });
      try {
  ...
  describe('invalidation', () => {
    ...
    it('drops every entry for a model', () => {
      cache.store('a', computeRequestHash('a', [{ role: 'user', content: 'q1' }]), makeResponse());
      cache.store('a', computeRequestHash('a', [{ role: 'user', content: 'q2' }]), makeResponse());
      cache.store('b', computeRequestHash('b', [{ role: 'user', content: 'q3' }]), makeResponse());
    ...
    it('clear() wipes everything', () => {
      cache.store('m', computeRequestHash('m', [{ role: 'user', content: 'q' }]), makeResponse());
      cache.clear();
