...
export const vueResolver: FrameworkResolver = {
  ...
  resolve(ref: UnresolvedRef, context: ResolutionContext): ResolvedRef | null {
    ...
    if (ref.referenceKind === "imports" && ref.referenceName.startsWith("#")) {
      if (
        NUXT_VIRTUAL_MODULES.some((prefix) =>
  ...
  extract(filePath: string, _content: string) {
    ...
    if (pagesIndex !== -1 && normalized.endsWith(".vue")) {
      const routePath = filePathToNuxtRoute(
        normalized,
    ...
    if (middlewareIndex !== -1) {
      const afterMiddleware = normalized.substring(
  ...
  for (const file of vueFiles) {
    ...
    if (componentName === name) {
      ...
      const component = nodes.find(
        (n) => n.kind === "component" && n.name === name,
      );
      if (component) {
        return component.id;
      }
 ...
 */
function filePathToNuxtRoute(
  normalized: string,
  afterPagesStart: number,
): string | null {
