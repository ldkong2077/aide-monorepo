...
class ClaudeCodeTarget implements AgentTarget {
  ...
  uninstall(loc: Location): WriteResult {
    ...
    // 1b. Also strip the AIDE entry from a legacy ./.claude.json so
    // uninstall fully reverses a pre-#207 local install. (Back when
    // the installer wrote the local MCP entry there, the project key
    // may still be `codegraph`; the cleanup helper handles both names
    ...
    // 2. Permissions — match either the new `mcp__aide__` prefix or
    // the legacy `mcp__codegraph__` prefix, so uninstall is idempotent
    // across the rename.
  ...
  printConfig(loc: Location): string {
    ...
    const snippet = JSON.stringify(
      { mcpServers: { aide: getMcpServerConfig() } },
      null,
      2,
    );
...
export function writeMcpEntry(loc: Location): WriteResult["files"][number] {
  ...
  // `./.mcp.json` locally) containing other MCP servers (no `aide`
  // key) is 'updated', not 'created' — we're adding an entry to a
  // file that was already there. Codex uses a different idiom
  // (empty-content => 'created') because its config.toml is ours
  // alone to manage.
...
/**
 * Strip the AIDE entry from a legacy project-local `./.claude.json`
 * (written by pre-#207 installers, which Claude Code never read).
 * Surgical: only our key (under either the new `aide` or the legacy
 * `codegraph` name) is removed; sibling MCP servers and any
 * unrelated keys are preserved, and the file is deleted only when
...
function cleanupLegacyLocalMcp(): WriteResult["files"][number] | null {
  ...
  if (!config.mcpServers?.aide && !config.mcpServers?.codegraph) return null;
  delete config.mcpServers.aide;
...
export function writePermissionsEntry(
  loc: Location,
): WriteResult["files"][number] {
  const file = settingsJsonPath(loc);
  const settings = readJsonFile(file);
  ...
  // Drop any legacy `mcp__codegraph__*` entries — those are stale and
  // would accumulate across the rename. We add the new `mcp__aide__*`
  // permissions below.
  settings.permissions.allow = settings.permissions.allow.filter(
    (p: string) => !p.startsWith("mcp__codegraph__"),
  );

  const want = getAidePermissions();
...
export function writeInstructionsEntry(
  loc: Location,
): WriteResult["files"][number] {
