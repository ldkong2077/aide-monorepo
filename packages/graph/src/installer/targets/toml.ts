 ...
 * single dotted-key table block (e.g. `[mcp_servers.aide]`) inside an
 ...
 *
 * Strategy: treat the file as text. Find the `[mcp_servers.aide]`
 ...
 * preserved verbatim, byte-for-byte. The header key is configurable
 * per call so the legacy `[mcp_servers.codegraph]` block can be
 * stripped during an upgrade.
 *
 ...
 *     the full dotted key, e.g. `[mcp_servers.aide]`).
 ...
 */
export function serializeTomlTableBody(
...
export function upsertTomlTable(
  ...
  const blockEnd = findNextTableHeader(
    fileContent,
    headerIdx + headerLine.length,
  );
  const existingBlock = fileContent
    .substring(headerIdx, blockEnd)
    .replace(/\n+$/, "");
