...
import { buildNode25BlockBanner, buildNodeTooOldBanner, MIN_NODE_MAJOR } from './node-version-check.js';

// Lazy-load heavy modules (CodeGraph, runInstaller) to keep CLI startup fast.
async function loadCodeGraph(): Promise<typeof import('../index.js')> {
  try {
...
if (process.argv.length === 2) {
  import('../installer/index.js').then(({ runInstaller }) =>
    runInstaller()
  ).catch((err) => {
...
// Imported at top of file from '../ui/shimmer-progress.js'
