...
- f44f7db: Add an async API alongside the existing sync helpers in
  `directory.ts`, so the MCP server and other async code can stop blocking the
  event loop.

  New exports:
  - `isInitializedAsync` / `createDirectoryAsync` / `removeDirectoryAsync`
  - `findNearestCodeGraphRootAsync` / `listDirectoryContentsAsync`
  - `getDirectorySizeAsync` / `ensureSubdirectoryAsync` /
    `validateDirectoryAsync`

  All eight functions mirror their sync counterparts and use `node:fs/promises`.
  The sync API is unchanged and marked `@deprecated` in JSDoc — it will be
  removed in v1.1.

  Added 15 tests in `directory.test.ts` covering the async path, including the
  symlink-safety guarantee (entries where `isSymbolicLink()` is true are always
