...
describe('countEnabledModes', () => {
  ...
  it('counts files (plural)', () => {
    expect(countEnabledModes({ files: 'a.ts,b.ts' })).toEqual(new Set(['files']));
  });

  it('counts pattern', () => {
    expect(countEnabledModes({ pattern: '*.ts' })).toEqual(new Set(['pattern']));
  });
...
describe('resolveVerifyTarget - mutual exclusion', () => {
  it('rejects --file + --files', async () => {
    await expect(resolveVerifyTarget({ file: 'a.ts', files: 'b.ts' })).rejects.toThrow(
      /Conflicting verify targets/,
    );
  });

  it('rejects --file + --pattern', async () => {
    await expect(resolveVerifyTarget({ file: 'a.ts', pattern: '*.ts' })).rejects.toThrow(
      /Conflicting verify targets/,
    );
  });

  it('rejects --file + --path', async () => {
    await expect(resolveVerifyTarget({ file: 'a.ts', path: '.' })).rejects.toThrow(
      /Conflicting verify targets/,
    );
  });

  it('rejects --file + --staged', async () => {
    await expect(resolveVerifyTarget({ file: 'a.ts', staged: true })).rejects.toThrow(
      /Conflicting verify targets/,
    );
  });

  it('rejects --path + --staged', async () => {
    await expect(resolveVerifyTarget({ path: '.', staged: true })).rejects.toThrow(
      /Conflicting verify targets/,
    );
  });

  it('rejects --base + --staged', async () => {
    await expect(resolveVerifyTarget({ staged: true }, 'HEAD~1')).rejects.toThrow(
      /Conflicting verify targets/,
    );
  });
});
...
describe('resolveVerifyTarget - file mode', () => {
  ...
  it('rejects non-existent --file', async () => {
    await expect(resolveVerifyTarget({ file: '/nonexistent/zzz.ts' })).rejects.toThrow(
...
describe('resolveVerifyTarget - files (plural) mode', () => {
  ...
  it('rejects --files with non-existent entry', async () => {
    await expect(resolveVerifyTarget({ files: '/nonexistent/x.ts' })).rejects.toThrow(
      /--files contains non-file entry|--files/,
    );
  });
});
...
describe('resolveVerifyTarget - pattern mode', () => {
  ...
  it('rejects --pattern that matches no files', async () => {
    ...
    try {
      process.chdir(dir);
      await expect(resolveVerifyTarget({ pattern: '*.nomatch' })).rejects.toThrow(
