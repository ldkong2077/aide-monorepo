...
import type { DashboardData, DashboardFilter } from "./types.js";
import type { DataCollector } from "./data.js";
import { createDataCollector } from "./data.js";
...
export class DashboardAPI {
  ...
  formatConsoleOutput(data: DashboardData): string {
    ...
    lines.push(
      `  Flows: ${data.summary.totalFlows} total, ${data.summary.activeFlows} active, ${data.summary.completedFlows} completed, ${data.summary.failedFlows} failed`,
    );
    lines.push(
      `  Tasks: ${data.summary.totalTasks} total, ${data.summary.completedTasks} completed, ${data.summary.failedTasks} failed`,
    );
    lines.push(
      `  Verifications: ${data.summary.totalVerifications} total, ${data.summary.trustVerdicts} TRUST, ${data.summary.reviewVerdicts} REVIEW, ${data.summary.rejectVerdicts} REJECT`,
    );
    lines.push(
      `  Costs: $${data.summary.estimatedCost.toFixed(2)} estimated, $${data.summary.actualCost.toFixed(2)} actual`,
    );
    lines.push("");
    ...
    if (data.flows.length > 0) {
      ...
      for (const flow of data.flows.slice(0, 5)) {
        ...
        lines.push(
          `     Status: ${flow.status} | Tasks: ${flow.tasksCompleted}/${flow.tasksTotal}`,
        );
      }
      lines.push("");
    }
  ...
  formatMarkdownOutput(data: DashboardData): string {
    ...
    if (data.flows.length > 0) {
      ...
      for (const flow of data.flows.slice(0, 10)) {
        lines.push(
          `| ${flow.projectName} | ${flow.status} | ${flow.progress}% | ${flow.tasksCompleted}/${flow.tasksTotal} |`,
        );
      }
      lines.push("");
    }
