...
export function calculateProgress(
  ...
  const completedTasksList = taskResults.filter(
...
function generateRecommendations(
  ...
  const tasksWithIssues = taskResults.filter(
    (t) =>
      t.verificationResult?.verdict === "REVIEW" ||
      t.verificationResult?.verdict === "REJECT",
  );
  ...
  if (completedCount === totalCount && totalCount > 0) {
    recommendations.push(
      "All tasks completed! Consider running a full verification",
    );
