 ...
 */
import type { Prompt, PromptMessage, GetPromptResult } from '@modelcontextprotocol/sdk/types.js';
...
export const PROMPTS: Prompt[] = [
  ...
  },
  {
    ...
    arguments: [
      { name: 'symbol', description: 'Symbol name to look up (e.g. "createProxyServer")', required: true },
      {
        name: 'project',
        description: 'Project root path (defaults to current directory)',
        required: false,
  ...
  },
  {
    ...
    arguments: [
      { name: 'file', description: 'File path to verify and fix', required: true },
    ],
  },
  {
 ...
 */
export function renderPrompt(name: string, args: Record<string, string> = {}): GetPromptResult | null {
  switch (name) {
...
function buildVerifyAndFixPrompt(args: Record<string, string>): GetPromptResult {
...
function buildIndexAndSummarisePrompt(args: Record<string, string>): GetPromptResult {
  const project = args.project ?? '.';
  const text = [
