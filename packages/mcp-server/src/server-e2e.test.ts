...
async function withServer<T>(
  fn: (send: (req: Record<string, unknown>) => Promise<JsonRpcResponse>) => Promise<T>,
): Promise<T> {
  ...
  let nextId = 1;
  const send = async (req: Record<string, unknown>): Promise<JsonRpcResponse> => {
    const id = nextId++;
    ...
    throw new Error(`Timed out waiting for response to ${req.method ?? JSON.stringify(req)}`);
  };
...
describe('MCP server (stdio e2e)', () => {
  ...
  it('lists all 4 tools over stdio', async () => {
    await withServer(async (send) => {
  ...
  it('lists all 3 resources over stdio', async () => {
    await withServer(async (send) => {
      ...
      expect(uris).toEqual(['aide://config', 'aide://graph/stats', 'aide://health']);
    });
  });
  ...
  it('reads the aide://health resource and returns valid JSON', async () => {
    await withServer(async (send) => {
      ...
      const res = await send({ method: 'resources/read', params: { uri: 'aide://health' } });
      expect(res.error).toBeUndefined();
      const contents = (res.result as { contents: { text: string; mimeType?: string }[] })
        .contents;
