...
describe('renderPrompt', () => {
  ...
  it('renders `code-review-with-aide` with files argument interpolated', () => {
    const result = renderPrompt('code-review-with-aide', { files: 'src/a.ts, src/b.ts' });
  ...
  it('renders `find-symbol-with-graph` with symbol argument interpolated', () => {
    const result = renderPrompt('find-symbol-with-graph', { symbol: 'createProxyServer' });
    expect(result).not.toBeNull();
    const text = (result!.messages[0].content as { type: string; text: string }).text;
  ...
  it('renders `index-and-summarise` with project argument interpolated', () => {
    const result = renderPrompt('index-and-summarise', { project: '/tmp/proj' });
    const text = (result!.messages[0].content as { type: string; text: string }).text;
