...
- f44f7db: Add graceful shutdown handling to the unified MCP server.

  The server now installs signal handlers for SIGINT and SIGTERM on startup. On
  signal, the server calls `Server.close()` to drain in-flight requests, then
  exits. A 5-second watchdog ensures the process exits even if `close()` hangs
  (e.g. a stuck stdio connection). A second signal during shutdown force-exits
  with code 1.

  This is required for clean `aide mcp serve` lifecycle management in
  containerised environments (k8s rolling updates, Docker stop, systemd
  shutdown).

  Added `installShutdownHandlers()` in `src/shutdown.ts` (also exported from the
  package root) with 7 unit tests covering:
  - signal registration / cleanup
  - close → log → exit on signal
  - idempotency (second signal is a no-op)
  - force-exit on hang
  - error logging when `close()` rejects
