...
echo "==> Installing @aide-dev/cli $AIDE_VERSION"
npm install -g --no-audit --no-fund "aide@${AIDE_VERSION/@aide\/cli@/@aide-dev/cli@}"
...
# npm's symlink dance.
AIDE_RESOLVED="$(npm root -g)/@aide-dev/cli"
if [[ ! -d "$AIDE_RESOLVED" ]]; then
  echo "ERROR: @aide-dev/cli was not installed to $AIDE_RESOLVED" >&2
  exit 1
fi
