...
if command -v npm >/dev/null 2>&1 && npm list -g @aide/cli >/dev/null 2>&1; then
  echo "==> Removing @aide/cli from the global npm prefix"
  npm uninstall -g --no-audit --no-fund @aide/cli || true
fi
